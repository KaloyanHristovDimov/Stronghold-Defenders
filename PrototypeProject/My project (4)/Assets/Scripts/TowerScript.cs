using System.Collections.Generic;
using UnityEngine;

public class TowereScript : MonoBehaviour
{
    private float attackTimer = 0f;
    public float attackTime = 5f;
    public List<GameObject> enemiesInRange = new List<GameObject>();
    public int damage = 100;
    public bool aoe = false;
    public float aoeRange = 5;
    private GameObject enemyToDamage;

    private LineRenderer lineRenderer;
    public AudioSource atackSFX;

    void Start()
    {
        lineRenderer = GetComponent<LineRenderer>();
        lineRenderer.enabled = false;
    }

    void Update()
    {
        attackTimer += Time.deltaTime;
        if (attackTimer >= attackTime)
        {
            attackTimer = 0f;
            Atack();
        }
    }
    private void Atack()
    {
        float MaxPathpoints = -1f;
        enemyToDamage = null;

        if (enemiesInRange.Count > 0)
        {
            for (int i = enemiesInRange.Count - 1; i >= 0; i--)
            {
                GameObject enemy = enemiesInRange[i];
                if (enemy != null)
                {
                    float pathPoints = enemy.GetComponent<EnemyScript>().pathPoints;
                    if (pathPoints > MaxPathpoints)
                    {
                        MaxPathpoints = pathPoints;
                        enemyToDamage = enemy;
                    }
                }
            }
            if (enemyToDamage != null)
            {
                int hp = enemyToDamage.GetComponent<EnemyScript>().health;
                if (hp - damage <= 0)
                {
                    RemoveFromList(enemyToDamage);
                }
                enemyToDamage.GetComponent<EnemyScript>().health = hp - damage;
                StartCoroutine(ShowShot(enemyToDamage.transform));
                atackSFX.Play();
            }
            else
            {
                RemoveFromList(enemyToDamage);
            }

        }


    }
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Enemy"))
        {
            GameObject enemy = other.gameObject;
            enemiesInRange.Add(enemy);
        }
    }
    private void OnTriggerExit(Collider other)
    {
        if (other.CompareTag("Enemy"))
        {
            RemoveFromList(other.gameObject);

        }
    }
    private void RemoveFromList(GameObject enemy)
    {
        enemiesInRange.Remove(enemy);
    }

    private System.Collections.IEnumerator ShowShot(Transform target)
    {
        lineRenderer.enabled = true;
        lineRenderer.SetPosition(0, transform.position);
        lineRenderer.SetPosition(1, target.position);

        yield return new WaitForSeconds(0.1f);

        lineRenderer.enabled = false;
    }

}